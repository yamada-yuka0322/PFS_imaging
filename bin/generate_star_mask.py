...
def main():
    ...
    outdir = config["bsmask"][f"{args.kind}_bsmask"]
    Path(outdir).mkdir(parents=True, exist_ok=True)

    starPath = config["Gaia_bs"]
    ...
    if (args.kind == 'target'):
        Downloads = loader.Target(config)
        print(f"{len(Downloads.ra)} targets downloaded")
    else:
        Downloads = None
    ...
    for tract in tract_list:
        ...
        _ra, _dec, _id = GetObjects(tract, args.kind, config, Downloads)
    ...
    if args.kind == "target" and len(all_tables) > 0:
        combined = vstack(all_tables)
        combined_path = os.path.join(outdir, "s23b_ssp_co_targets_bsmask.fits")
...
def GetObjects(tract, kind, config, Downloads):
    """
    ...
    """
    if(kind == 'target'):
        ra, dec, _, _, ID = Downloads.get_tract(int(tract))
        print(f"{len(ra)} targets in tract{tract}")
    elif(kind=='star'):
        Downloads = loader.Star(tract, config)
        ra = Downloads.ra
        dec = Downloads.dec
        ID = Downloads.objectID
    elif(kind=='random'):
        Downloads = loader.Random(tract, config)
        ra = Downloads.ra
        dec = Downloads.dec
        ID = Downloads.objectID
