...
def plot_map(df, target, field, ax, vmin=None, vmax=None, cmap='viridis'):
    """
    input
    ------------------------------------------------------
    df: pandas dataframe
    ...
    for pix, val in zip(valid_pix, values):
        ...
        vec = hp.ang2vec(*hp.pix2ang(nside, pix))  # 中心ベクトル
        theta, phi = hp.vec2ang(boundary.T)
        ra, dec = hp.pix2ang(nside, pix, lonlat=True)  # 中心座標（描画用制限に使う）
    ...
    coll = PolyCollection(
        ...
        edgecolors='none', linewidth=0, norm=norm  # ← norm を指定
    )
    ax.add_collection(coll)

    # 軸設定
    ax.set_xlim(ramin, ramax)  # RA範囲
...
###############################################################################################################
def plot_ang(target_list = [], field="all"):
    """
    input
    -------------------------------------------------------
    df: pandas dataframe
    ...
    data = prepare_data(cut_edge=True, do_jackknife=True, target_list = target_list)
    mean_density = np.sum(data['target']*data['area'])/np.sum(data['area'])
    
    keys = ["gseeing", "rseeing", "iseeing", "zseeing", "yseeing", "g_depth", "r_depth", "i_depth", "z_depth", "y_depth", "extinction", "star"]
...
#####################################################################################################
def plot_dens(target_list = [], field="all"):
...
#####################################################################################################
def plot_PS(target_list = [], random=True, field="all"):
    data = prepare_data(cut_edge=True, do_jackknife=True, target_list = target_list)
    mean_density = np.sum(data['target']*data['area'])/np.sum(data['area'])
...
def prepare_data(cut_edge, do_jackknife, target_list = [], field = 'all'):
    if cut_edge:
        _autumn, _spring = mask_edge()
